...
exports.getUserSettings = async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === "admin") {
      // 🔥 OPTIMIZED: Already perfect with .select() and .lean()
...
exports.getUserSessions = async (req, res) => {
  try {
    ...
    // ==========================================
    if (req.user && req.user.role === "admin") {
      // 🔥 OPTIMIZED: Added .select() so we only fetch exactly what the map function needs
      const activeAdminSessions = await AdminSession.find({ admin: userId })
        .select("deviceInfo location ipAddress createdAt token") 
        .sort({ createdAt: -1 })
        .lean();
      ...
      // Return here to prevent executing the user logic below!
